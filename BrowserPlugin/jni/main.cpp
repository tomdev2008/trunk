...
EXPORT NPError NP_Initialize(NPNetscapeFuncs* browserFuncs,
		NPPluginFuncs* pluginFuncs, void *java_env);
...
NPError NP_Initialize(NPNetscapeFuncs* browserFuncs, NPPluginFuncs* pluginFuncs,
		void *java_env) {
	gLogI.log(kDebug_ANPLogType, "NP_Initialize()");
	...
	static const struct {
		...
		uint32_t size;
		ANPInterface* i;
	} gPairs[] = { { kAudioTrackInterfaceV0_ANPGetValue, sizeof(gSoundI),
			&gSoundI }, { kBitmapInterfaceV0_ANPGetValue, sizeof(gBitmapI),
			&gBitmapI }, { kCanvasInterfaceV0_ANPGetValue, sizeof(gCanvasI),
			&gCanvasI }, { kEventInterfaceV0_ANPGetValue, sizeof(gEventI),
			&gEventI }, { kLogInterfaceV0_ANPGetValue, sizeof(gLogI), &gLogI },
			{ kPaintInterfaceV0_ANPGetValue, sizeof(gPaintI), &gPaintI }, {
					kPathInterfaceV0_ANPGetValue, sizeof(gPathI), &gPathI }, {
					kSurfaceInterfaceV0_ANPGetValue, sizeof(gSurfaceI),
					&gSurfaceI }, { kSystemInterfaceV0_ANPGetValue,
					sizeof(gSystemI), &gSystemI }, {
					kTypefaceInterfaceV0_ANPGetValue, sizeof(gTypefaceI),
					&gTypefaceI }, { kWindowInterfaceV0_ANPGetValue,
					sizeof(gWindowI), &gWindowI }, };
...
NPError NPP_New(NPMIMEType pluginType, NPP instance, uint16 mode, int16 argc,
	...
	gLogI.log(kDebug_ANPLogType, "NPP_New()");
	...
	unsigned int modelsBits;
	NPError errDM = browser->getvalue(instance,
			kSupportedDrawingModel_ANPGetValue, &modelsBits);
	if (errDM) {
		gLogI.log(kError_ANPLogType, "get model err %d", errDM);
		return errDM;
	}

	if (modelsBits & kBitmap_ANPDrawingModel) {
		gLogI.log(kDebug_ANPLogType,
				"NPP_New()  support kBitmapUC_ANPDrawingModel");
		model = kBitmap_ANPDrawingModel;
	} else {
		gLogI.log(kDebug_ANPLogType,
				"NPP_New()  not support kBitmapUC_ANPDrawingModel");
	...
	for (int i = 0; i < argc; i++) {
		gLogI.log(kDebug_ANPLogType, "name:%s, value:%s", argn[i], argv[i]);
	}

	for (int i = 0; i < argc; i++) {
		if (!strcmp(argn[i], "paydata")) {
			payData = argv[i];
			break;
		}
	}
	...
	gLogI.log(kDebug_ANPLogType, "------ %p PluginType is %d", instance,
			obj->pluginType);

	// check to ensure the pluginType supports the model
	if (!obj->activePlugin->supportsDrawingModel(model)) {
		gLogI.log(kError_ANPLogType, "------ %p Unsupported DrawingModel (%d)",
				instance, model);
		return NPERR_GENERIC_ERROR;
	}
	return NPERR_NO_ERROR;
...
EXPORT NPError NP_GetValue(NPP instance, NPPVariable variable, void *value) {
	...
	gLogI.log(kDebug_ANPLogType, "NP_GetValue");
	return NPERR_GENERIC_ERROR;
}

NPError NPP_GetValue(NPP instance, NPPVariable variable, void* value) {
	gLogI.log(kDebug_ANPLogType, "NPP_GetValue~~~");
	...
	if (variable == kJavaSurface_ANPGetValue ) {
		...
		gLogI.log(kDebug_ANPLogType, "NPP_GetValue(kJavaSurface_ANPGetValue)");

		//get the surface sub-plugin
		PluginObject* obj = static_cast<PluginObject*>(instance->pdata);
		if (obj && obj->activePlugin) {

			if (obj->activePlugin->supportsDrawingModel(
					kSurface_ANPDrawingModel)) {
				SurfaceSubPlugin* plugin =
						static_cast<SurfaceSubPlugin*>(obj->activePlugin);
				jobject* surface = static_cast<jobject*>(value);
				*surface = plugin->getSurface();
				gLogI.log(kDebug_ANPLogType,
						"NPP_GetValue(kJavaSurface_ANPGetValue)  Surface  value:0x%x",
						surface);
				return NPERR_NO_ERROR;
			} else {
